    ...
    while True:
        ...
        while i < min(len(caminos1[0]), len(caminos2[0])):
            ...
            if siguiente_pos1 and siguiente_pos2 and detectar_colision(siguiente_pos1, siguiente_pos2):
                ...
                # Recalcular la ruta del montacargas 1 para evitar la posición del montacargas 2
                tablero_con_obstaculo = np.copy(tablero)
                tablero_con_obstaculo[siguiente_pos2] = 1  # Añadir obstáculo donde está el segundo montacargas
                a_estrella1 = AEstrella(tablero_con_obstaculo)
                caminos1, costo1 = a_estrella1.buscar_camino(objetivos1)
