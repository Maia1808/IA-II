...
def menu():
    ...
    while True:
        ...
        def update(frame):
            ...
            # Verificar si el Montacargas 1 debe esperar antes de moverse
            siguiente_pos1 = caminos1[0][frame + 1] if frame + 1 < len(caminos1[0]) else None
            siguiente_pos2 = caminos2[0][frame + 1] if frame + 1 < len(caminos2[0]) else None

            # Si hay colisión, Montacargas 1 debe esperar
            if siguiente_pos1 and siguiente_pos2 and detectar_colision(siguiente_pos1, siguiente_pos2):
                if not en_espera1:
                    en_espera1 = True  # Comienza el estado de espera para Montacargas 1
                    print(f"Montacargas 1 espera en la casilla {caminos1[0][frame]}")
            
            if en_espera1:
                # Montacargas 1 no se mueve en esta iteración, espera hasta la siguiente
                if frame < len(caminos1[0]):
                    ax.plot(caminos1[0][frame][1] + 0.5, caminos1[0][frame][0] + 0.5, 'bo', markersize=10)  # Montacargas 1 (azul)
                en_espera1 = False  # Termina la espera en el siguiente ciclo
            else:
                if frame < len(caminos1[0]):
                    ax.plot(caminos1[0][frame][1] + 0.5, caminos1[0][frame][0] + 0.5, 'bo', markersize=10)  # Montacargas 1 (azul)

            # Dibujar los caminos de los montacargas
